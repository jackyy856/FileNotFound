...
// Dim world + panel
draw_set_alpha(0.62); draw_set_color(c_black); draw_rectangle(0,0,GW,GH,false); draw_set_alpha(1);
draw_set_color(make_color_rgb(28,32,38)); draw_roundrect(0,0,GW,GH,false);
...
if (state == "root") {
    for (var i = 0; i < array_length(buttons_root); i++) {
        ...
        var over = _hit(b, device_mouse_x_to_gui(0), device_mouse_y_to_gui(0));
...
// Back button shown for every submenu
var bb = btn_back;
...
if (state == "submenu") {
    if (submenu == "load" || submenu == "save") {
        ...
        for (var s = 0; s < array_length(slots); s++) {
            var sl = slots[s];
            var over = _hit(sl, device_mouse_x_to_gui(0), device_mouse_y_to_gui(0));
            draw_set_color(over ? make_color_rgb(80,170,120) : make_color_rgb(230,230,230));
