// Persistent ESC menu overlay for Save/Load/Settings/Main Menu
persistent = true;

// ---------- STATE ----------
active   = false;                 // overlay visible
state    = "root";                // "root" | "submenu" | "confirm" | "toast"
submenu  = "";                    // for "submenu": "load" | "save" | "settings" | "mainmenu"
toast_txt = ""; toast_t = 0;
...
// we’ll rebuild positions whenever GUI size changes
_last_gui_w = -1;
_last_gui_h = -1;

// Recompute every layout number from the current GUI size
function _layout() {
    ...
    col_x = (GW - btn_w) * 0.5;
    col_y = (GH * 0.5) - (btn_h*2 + btn_gap*1.5);

    // ROOT buttons (note: kinds must match Step’s switch)
    buttons_root = [
    ...
    // Load/Save grid (3 slots)
    slot_w = 360; slot_h = 280; slot_gap = 56;
...
function _save_write(slot_idx) {
    ...
    var when  = date_current_datetime();
    var title = date_format_datetime(when, "%Y-%m-%d %H:%i:%s");
...
function _save_load(slot_idx) {
    ...
    if (!is_undefined(global.save_extra_apply)) global.save_extra_apply(pack);
